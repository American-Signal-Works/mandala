...
export function CliAuthorizeFlow({
  ...
  const [selectedCompanyId, setSelectedCompanyId] = useState(
    inspection?.selectedCompanyId ?? companies[0]?.id ?? ""
  )
  ...
  async function decide(decision: FinishedState) {
    ...
    try {
      ...
      if (
        !response.ok ||
        !cliDeviceAuthorizationDecisionResponseSchema.safeParse(body).success
      ) {
    ...
    <Card>
      <CardHeader className="border-b">
        ...
        <CardDescription className="text-pretty">
          Confirm that the device details below match the terminal you just
          used. Signed in as {signedInEmail ?? "your Mandala account"}.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="flex gap-3 text-sm text-muted-foreground">
          ...
          <p className="text-pretty">
            This grants the CLI access to one workspace. It does not approve
            actions or bypass Mandala&apos;s normal controls.
          </p>
        </div>
