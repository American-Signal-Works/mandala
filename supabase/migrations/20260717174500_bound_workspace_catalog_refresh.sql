-- Keep workspace catalog refresh proportional to changed datasets instead of
-- expanding every JSON payload in the company on every Sandbox run.
--
-- Exact record counts and freshness still come from all rows. Schema discovery
-- is bounded to the 256 most recently changed records per pending dataset, and
-- the schema hash intentionally excludes observed counts so ordinary volume
-- changes do not masquerade as schema drift.

create index if not exists external_records_catalog_refresh_idx
  ...
  with target_scopes as materialized (
    ...
    where record.company_id = p_company_id
      and not exists (
        select 1
        from public.workspace_data_catalogs catalog
        where catalog.company_id = record.company_id
          and catalog.source_id = record.source_id
          and catalog.record_type = record.record_type
      )
    group by
      record.company_id,
      record.source_id,
    ...
    join public.external_records source_record
      on source_record.company_id = scope.company_id
    ...
    join public.external_record_links link
      on link.from_record_id = source_record.id
     and link.company_id = source_record.company_id
    ...
    select
      ...
      scope.source_key,
      scope.record_type,
      count(record.id)::bigint as record_count,
    ...
    join public.external_records record
      on record.company_id = scope.company_id
     and record.source_id = scope.source_id
     and record.record_type = scope.record_type
