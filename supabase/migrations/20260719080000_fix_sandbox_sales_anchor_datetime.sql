-- Real connector sales orders carry ISO datetime order_date values
-- (e.g. 2026-04-01T23:18:57). The anchored date-only pattern rejected every
-- real row, so the sales anchor and 30-day velocity silently computed as
-- empty (dataAnchorAt null, recentSalesUnits 0 for all candidates). Accept a
-- leading date and let the existing ::DATE casts truncate the time component.

CREATE OR REPLACE FUNCTION sandbox_private.build_workspace_snapshot_v1(p_company_id uuid, p_candidate_limit integer DEFAULT 25)
 RETURNS jsonb
 LANGUAGE sql
